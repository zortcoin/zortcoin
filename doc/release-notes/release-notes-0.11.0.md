Zortcoin Core version 0.11.0 is now available from:

  <https://zortcoin.org/bin/zortcoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer ZTC/kB fee to be
...
For example, add the following to `zortcoin.conf`:
...
related to the blockchain in the zortcoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Zortcoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Zortcoin Core will 
...
For an example script see [zortcoin-submittx](https://github.com/laanwj/zortcoin-submittx).
...
- #5334 `cf87536` libzortcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new zortcoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only ZortcoinAmountField
- #5753 `0f44672` Add zortcoin logo to about screen
...
- #5809 `46bfbe7` Add zortcoin-cli man page
...
- #5849 `41259ca` contrib/init/zortcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Zortcoin-Qt.app to Zortcoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for zortcoin-tx)
