...
The Zortcoin Core daemon can be configured to act as a trusted "border
router", implementing the zortcoin wire protocol and relay, making
...
The ZeroMQ feature in Zortcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building zortcoind:
...
For instance:
    ...
    $ zortcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/zortcoind.tx.raw \
...
These options can also be provided in zortcoin.conf.
...
From the perspective of zortcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into zortcoind directly. Furthermore, no information is
...
using. Zortcoind appends an up-counting sequence number to each
