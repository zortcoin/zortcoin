...
const char * const ZORTCOIn_CONF_FILENAME = "zortcoin.conf";
const char * const ZORTCOIn_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/zortcoin/zortcoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //zortcoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", ZORTCOIn_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Zortcoin
    // macOS: ~/Library/Application Support/Zortcoin
    // Unix-like: ~/.zortcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Zortcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Zortcoin";
#else
    // Unix-like
    return pathRet / ".zortcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", ZORTCOIn_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Zortcoin Core copyright is not removed by accident
    if (copyright_devs.find("Zortcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Zortcoin Core developers";
    }
